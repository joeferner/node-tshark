...
PcapConverter.prototype.convertStream = function (fileName) {
  ...
  parser.on('globalHeader', function (header) {
    ...
    parser.on('packet', function (packet) {
      var pcapPacketHeaderBuffer = createPacketHeaderBuffer(header, packet.header);

      var totalPacketBufferLength = pcapPacketHeaderBuffer.length + packet.data.length;
      var packetBuffer = Buffer.concat([pcapPacketHeaderBuffer, packet.data], totalPacketBufferLength);
...
PcapConverter.prototype.createStream = function (fileName) {
  ...
  // pcapConverterStream.pause = function () {
  //   console.error('PcapConverter pause called');
  // };

  // pcapConverterStream.resume = function () {
  //   console.error('PcapConverter resume called!');
  // };

  var q = async.queue(function (task, callback) {
    var packetCount = task.packetCount;
    var packetName = 'packet ' + packetCount;
    var packet = task.packet;
    var globalHeader = task.globalHeader;

    // console.error('gogo ' + packetName);
    ...
    function onWriteFile(writeFileErr) {
      ...
      tshark.on('exit', onTSharkExit.bind(undefined, tsharkPacketBuffers));
    }
    ...
    function onTSharkExit(tsharkPacketBuffers, code) {
      if (code) {
  ...
  parser.on('globalHeader', function (globalHeader) {
    ...
    parser.on('packet', function (packet) {
      packetCount++;
      // console.error('pushing packet ' + packetCount + ' to q.');
      ...
      q.push({
        ...
        // console.error('finished processing packet!');
      });
