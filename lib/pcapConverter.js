...
var queueHist = new metrics.Histogram.createUniformHistogram();
var dataMetric = new metrics.Timer();
metricsServer.addMetric('queue', queueHist);
metricsServer.addMetric('data', dataMetric);
...
PcapConverter.prototype.convertStream = function (inputStream) {
  ...
  var timer = setInterval(function () {
    queueHist.update(q.length());
    ++secondsPassed;
    // console.log("Seconds Passed:", ++secondsPassed);
    // console.log("Packets Obtained:", packetCount);
    // console.log("Packets Ever Received", allthepackets);



    if ((secondsPassed >= 5 && packetCount > 0) || packetCount >= 1000) {
      packetCount = 0;
      secondsPassed = 0;
  ...
  function processPacket(packet, callback) {
    ...
    if (!fileStarted) {
      return writePacketToTempFile(globalHeader, packet, function(err, tempFilePath) {
        ...
        ++packetCount;
        dataMetric.update(packet.header.capturedLength);
        fileStarted = true;
...
function outputMetrics() {
  var date = new Date();
  //Log the metrics
  var totalPacketCount = dataMetric.histogram.count;
  //packetRate is in packets/sec
  var packetRate = Math.round(dataMetric.meter.meanRate());
  //Data Rate is in bytes/sec
  var avgDataRate = Math.round(packetRate * dataMetric.histogram.mean());
  var maxQueueSize = queueHist.max;
  var avgQueueSize = Math.round(queueHist.mean());
  // console.log(totalPacketCount, packetRate, avgDataRate, avgQueueSize, maxQueueSize);
  var logString = date + 
    "\n Total Packets Processed: " + totalPacketCount + 
                "\n Packet Rate: " + packetRate + " packets/second" +
          "\n Average Data Rate: " + avgDataRate + " bytes/second" +
  ...
  // fs.open(metricsFile, 'a', function (err, fd) {
  //   fd.appendFile
  // });

  setTimeout(outputMetrics, 3000);
}
