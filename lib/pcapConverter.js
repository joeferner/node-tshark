...
var PcapConverter = module.exports = function() {
  ...
  this.metrics = {
    queue: new metrics.Histogram.createUniformHistogram(),
    data: new metrics.Timer(),
    packetCount: new metrics.Counter(),
    startTime: new Date()
  };
};
...
PcapConverter.prototype.convertStream = function(inputStream) {
  ...
  var q = async.queue(function(packet, callback) {
  ...
  function processGroupOfPackets() {
    self.metrics.queue.update(q.length());
    if (!tempFile) {
      if (q.length() == 0 && parseOnEndOccured) {
        self.emit('end');
      }
      return;
    }
    ...
    if ((timePassed >= 5000 && tempFile.packetCount > 0) || tempFile.packetCount >= 1000) {
      ...
      tempFile = null;
      return processFile(oldTempFile, function(err, data) {
        if (err) {
          self.emit('error', err);
        }
        self.emit('data', data);
    ...
    } else if (q.length() == 0 && parseOnEndOccured) {
      self.emit('end');
    }
  }

  function processFile(tempFile, callback) {
    return fs.close(tempFile.fd, function(err) {
      if (err) {
        return callback(err);
      }

      return runTShark(tempFile.path.toString(), function(err, results, oldName) {
        if (err) {
          return callback(err);
        }
  ...
  function processPacket(packet, callback) {
    ...
    return getOrOpenTempFile(function(err, tempFile) {
      ...
      return writePacket(tempFile, packet, function(err) {
        ...
        self.metrics.data.update(packet.header.capturedLength);
        self.metrics.packetCount.inc(1);
